def assign_keys(networks):
    ...
    Each person gets the key in the lowest available slot on their
    device (1 to 4).

    Parameters:
        networks (list of tuple): Each tuple represents a network of
            people.

    Returns:
        list of dict: Each dict has the network number, key id, and a
            mapping of person to slot.

    Raises:
        Exception: If any person is assigned more than 4 keys (their device
            is full).
    """
...
def print_assignments(assignments):
    ...
    for assignment in assignments:
        print(
            f"Network {assignment['network']}, key {assignment['key']} -"
        )
        for person, slot in assignment["members"].items():
            print(
                f"{person} - put key {assignment['key']} in slot {slot}"
            )
        print()
